            ...
            <form action="{{ route('admin.profile.update', $profile->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div id="history-container" class="mb-4">
                    @foreach (json_decode($profile->history ?? '[]', true) as $index => $item)
                        <div class="mb-2">
                            <input type="hidden" name="history[]" id="history-{{ $index }}"
                                value="{{ $item }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="text" name="vidio" class="form-control"
                                value="{{ old('vidio', $profile->vidio) }}">
                        </div>
                        {{-- Struktur Organisasi (gambar tunggal) --}}
                        <div class="col-md-6 mb-4">
                            <label>Struktur Organisasi</label>
                            <img class="mb-3" id="preview"
                                src="{{ $profile->organizational_structure ? asset('storage/' . $profile->organizational_structure) : '#' }}"
                                alt="Preview"
                                style="max-width: 100%; {{ $profile->organizational_structure ? '' : 'display:none;' }}">
...
@section('scripts')
    <script>
        let trixIndex =
            {{ is_array(json_decode($profile->history ?? '[]', true)) ? count(json_decode($profile->history ?? '[]', true)) : 0 }};
