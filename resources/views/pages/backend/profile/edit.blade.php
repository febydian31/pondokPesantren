    ...
    <div class="card-body">
        <form action="{{ route('profile.update', $profile->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div id="history-container" class="mb-4">
                @foreach($profile->history as $index => $item)
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-4">
                    ...
                    <div id="organizational_structure-container">
                        @foreach($profile->organizational_structure as $image)
                            <div class="input-group mb-2 align-items-center">
                                <input type="file" name="organizational_structure[]" class="form-control me-2" accept="image/*" onchange="previewImage(this)">
                                <button type="button" class="btn btn-danger" onclick="removeField(this)">×</button>
                                @if($image)
                                    <img src="{{ asset('storage/' . $image) }}" alt="Preview" class="ms-2" style="max-width: 120px;">
                                @endif
                            </div>
                        @endforeach
                    </div>
...
@section('scripts')
<script>
let trixIndex = {{ count($profile->history) }};
